...
├── public/
│   └── docs/
│       ├── api-docs.json          ← OpenAPI 3.0.0 spec (manually maintained)
│       └── index.html             ← Swagger UI (CDN-based)
├── CLAUDE.md
├── project_spec.md
├── architecture.md
└── project_state.md
...
| POST   | /v1/inspections/{id}/approve            | InspectionController@approve (role:supervisor,admin) |
| POST   | /v1/inspections/{id}/return             | InspectionController@returnInspection (role:supervisor,admin) |
...
**Total: 54 routes** (including PATCH variants generated by apiResource alongside PUT)
...
2. **Supervisor user**: name="Supervisor Demo", email=supervisor@americanadvisor.com, password=password, role=supervisor
3. **Inspector user**: name="Inspector Demo", email=inspector@americanadvisor.com, password=password, role=inspector
...
- **OpenAPI 3.0.0 spec**: `public/docs/api-docs.json` — manually maintained, covers all 52 endpoints
- **Swagger UI**: `http://localhost:8000/docs/index.html` — interactive API explorer with Try-it-out
- **No package dependency**: Uses Swagger UI from CDN (`swagger-ui-dist@5`)
- **Auth in Swagger**: Click "Authorize" button, paste Bearer token from `/v1/login` response
- **Maintenance**: When adding/modifying endpoints, update `api-docs.json` manually. Spec includes 30+ component schemas, 11 tag groups, and full request/response examples.
